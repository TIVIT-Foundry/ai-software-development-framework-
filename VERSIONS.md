...
|---------|-------|--------|-------|
| 4.3.6 | 2026-08-14 | 114 | Hardening de Spec Kit y determinismo: validador de drift de especificaciones (`check-spec-drift.py`, 16 checks totales), verificador de gates de fase (`check-phase-gate.ps1`), auto-sincronización bidireccional de tareas `tasks.md` ↔ `.workflow/state.json` en `generate-progress.py`, template CI para GitHub Actions (`framework-ci.yml`), soporte `-IncludeCI` y auto-deploy de CI en `update-framework.ps1` |
