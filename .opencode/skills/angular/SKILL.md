...
Define the architecture and conventions for Angular applications in the framework. Covers standalone components, signals, dependency injection, routing, lifecycle hooks, template syntax, change detection strategies, lazy loading, and Angular CDK integration. This skill replaces the former `react` skill to align with the company's Angular stack.
