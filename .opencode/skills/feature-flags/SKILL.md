...
Esta skill complementa `angular` (componentes, directivas, servicios) y `ci-cd` (pipelines). Mientras aquellos definen la UI y el deploy, esta skill define qué features son visibles para quiénes y cuándo.
...
- `angular` define los componentes y directivas que esta skill condiciona con flags.
...
3. Crear la directiva `*featureFlag` para Angular.
...
- estructura de componentes y servicios (`angular`);
...
- la directiva wrapper `*featureFlag`;
- la convención de naming;
- el lifecycle de gestión;

Esta skill delega:
- la estructura de componentes y servicios a `angular`;
...
### 3. Directiva wrapper `*featureFlag`
...
### 4. Servicio `FeatureFlagsService`
...
### 5. InjectionToken + Provider con soporte para targeting
...
### 6. Convención de naming de flags
...
### 7. Lifecycle de una feature flag
    ...
    steps:
      ...
      - name: Check feature flags
        run: |
          ...
          grep -r "enable-\|disable-\|exp-\|ops-\|beta-" src/ --include="*.ts" --include="*.html" | \
...
- Archivo `src/features/flags/feature-flags.service.ts` con servicio Angular.
- Archivo `src/features/flags/feature-flag.directive.ts` con directiva `*featureFlag`.
- Archivo `src/features/flags/feature-flags.provider.ts` con `provideFeatureFlags`.
...
- `angular` consume la directiva `*featureFlag` y el servicio `FeatureFlagsService`;
...
- La directiva `*featureFlag` tiene fallback explícito.
- El servicio `FeatureFlagsService` retorna el tipo correcto, no `any`.
...
Cuando el usuario use un `if (isBetaUser)` hardcodeado, el agente debe proponer la directiva `*featureFlag` y explicar la ventaja.  
...
- ¿Se creó la directiva `*featureFlag` con fallback?
- ¿Se creó el servicio `FeatureFlagsService` con tipado estricto?
- ¿Se configuró el provider con targeting rules?
